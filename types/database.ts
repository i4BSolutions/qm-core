...
// ============================================
// Permission Matrix Types (v1.13)
// ============================================

/**
 * The 16 protected resources in the QM permission matrix.
 * Mirrors the permission_resource enum in the database.
 * Phase 61 (Permission Management UI) uses PERMISSION_RESOURCES to render the matrix grid.
 */
 ...
 * Mirrors the permission_level enum in the database.
 *   edit  = full CRUD (approve/reject for SOR approval layers)
 *   view  = read-only (can see records but cannot act)
 *   block = no access (resource hidden, no rows returned by RLS)
 */
export type PermissionLevel = 'edit' | 'view' | 'block';

/**
 * Single row from the user_permissions table.
 * Every user has exactly 16 rows — one per PermissionResource.
 */
...
/** Insert shape — id, created_at, updated_at are generated by the database. */
export type UserPermissionInsert = Omit<UserPermission, 'id' | 'created_at' | 'updated_at'>;

/** Update shape — only level is writable after creation. */
export type UserPermissionUpdate = Partial<Pick<UserPermission, 'level'>>;

/**
 * All 16 permission resources in display order.
 * Use this array to iterate over resources in UI grids (Phase 61).
 */
...
/** Human-readable display names for each resource (used in Permission Management UI). */
...
/** Human-readable display names for each permission level. */
