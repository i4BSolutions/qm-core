...
- Initialize `approvalData` for each line item with:
  ...
  - `warehouseId` empty
- Fetch warehouse stock levels for each unique item_id:
  - Query `inventory_transactions` grouped by warehouse, calculate in/out net per warehouse
  - Same pattern as existing stock-out page's `fetchItemStock` function
  - Store in `warehouseStocks` map keyed by item_id
