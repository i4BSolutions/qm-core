...
must_haves:
  truths:
    ...
    - "PO Edit button is visible only when user has permission AND status is not closed/cancelled AND user is not Quartermaster"
  ...
  artifacts:
    ...
    - path: "app/(dashboard)/po/[id]/page.tsx"
      provides: "Permission-gated PO Edit button with status check and Quartermaster exclusion"
      contains: "!isQuartermaster"
  ...
  key_links:
    ...
    - from: "app/(dashboard)/po/[id]/page.tsx"
      to: "lib/hooks/use-permissions.ts"
      via: "isQuartermaster check"
      pattern: "isQuartermaster"
---

<objective>
Add permission-gated Edit buttons to QMRL, QMHQ, and PO detail pages, and remove Edit button from Invoice detail page.

Purpose: Users can only see Edit buttons for entities they have permission to modify. PO Edit button respects both permission and status (hidden when closed/cancelled), and explicitly excludes Quartermaster role per user decision. Invoice has no Edit button since void functionality serves as the modification mechanism.
...
<task type="auto">
  <name>Task 2: Add permission check to PO Edit button with Quartermaster exclusion</name>
  <files>
    app/(dashboard)/po/[id]/page.tsx
  </files>
  <action>
    **PO Detail Page (`po/[id]/page.tsx`):**
    1. Import `usePermissions` hook from `@/lib/hooks/use-permissions`
    2. Add `const { can, isQuartermaster } = usePermissions();` in component (near other hooks)
       - IMPORTANT: Destructure BOTH `can` AND `isQuartermaster` from the hook
    3. Update `showEditButton` logic (around line 214) to include permission check AND explicit Quartermaster exclusion:
       ```tsx
       ...
       ```
       - WHY: The permission matrix grants Quartermaster CRUD on purchase_orders, but the user explicitly decided "Quartermaster: Cannot edit PO" in CONTEXT.md. This is an explicit override.
  ...
  <verify>
    ...
    4. Verify Quartermaster exclusion: When logged in as Quartermaster, Edit button should NOT appear on any PO
  </verify>
  <done>
    - PO Edit button only visible when ALL three conditions are met:
      1. User has `update` permission on `purchase_orders`
      2. User is NOT a Quartermaster (explicit exclusion per user decision)
      3. PO status is not `closed` or `cancelled`
...
- PO detail page: Edit button visible only for users with purchase_orders update permission AND user is NOT Quartermaster AND when PO is not closed/cancelled
...
3. PO Edit button respects `can("update", "purchase_orders")` AND `!isQuartermaster` AND `canEditPO(status)`
