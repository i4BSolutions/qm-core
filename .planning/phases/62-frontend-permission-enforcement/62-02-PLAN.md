...
4. **lib/hooks/use-permissions.ts:**
   - This file imports `UserRole` from `@/types/database` (or `@/types`), which will be removed from `types/database.ts` in step 5 below. The import MUST be updated before or alongside that deletion to avoid a type-check failure.
   - Remove the line `import type { UserRole } from "@/types/database"` (or `@/types`) from the top of the file.
   - Add a local type alias at the top of the file to replace it:
     ```typescript
     // Kept local after UserRole was removed from types/database.ts in Phase 62
     type UserRole = "admin" | "qmrl" | "qmhq";
     ```
   - All other code in the file (the `PermissionMatrix` type, `permissionMatrix` constant, `usePermissions()` function, and `useResourcePermissions()` function) is left unchanged. The deprecated `usePermissions()` function stays in the file as a dead-code fallback; Task 1 already removes all its call sites.
   - Mark `usePermissions` with `@deprecated` JSDoc if it is not already annotated.
...
5. **types/database.ts:**
   ...
   - IMPORTANT: Before removing, grep for all `UserRole` imports across the codebase. After step 4 above, `lib/hooks/use-permissions.ts` will no longer import it. Remaining expected consumers:
     - `components/providers/auth-provider.tsx` — has `useUserRole()` that returns `null as UserRole | null`. Update to inline the type directly: `null as ("admin" | "qmrl" | "qmhq") | null`, or simply return `null` if the hook is unused.
   - If there are any other files still importing `UserRole` after the above changes, update them to use the inline union type `"admin" | "qmrl" | "qmhq"` directly.
   - The safest approach: remove `UserRole` from the exported types and ensure `types/index.ts` no longer re-exports it.
  </action>
  <verify>
1. `npm run type-check` passes
2. `npm run build` passes
