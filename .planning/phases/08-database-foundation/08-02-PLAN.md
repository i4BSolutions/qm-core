...
must_haves:
  truths:
    ...
    - "Balance in Hand correctly remains unchanged when invoice is voided (PO commitment unchanged by design)"
  artifacts:
...
Purpose: When an invoice is voided, the database must (1) block the void if stock has been received, and (2) log all cascade effects (PO status changes, invoiced quantity changes) to the audit trail. This ensures financial integrity and auditability. Note: Balance in Hand does NOT change when an invoice is voided because the PO commitment remains unchanged by design.
...
1. Check trigger exists and is ordered correctly:
```sql
SELECT tgname, tgtype FROM pg_trigger
WHERE tgrelid = 'invoices'::regclass
  AND tgname LIKE '%void%' OR tgname LIKE '%invoice%'
ORDER BY tgname;
```
The aa_block_invoice_void_stockin trigger should appear first alphabetically.

2. Check function exists:
```sql
SELECT proname FROM pg_proc WHERE proname = 'block_invoice_void_with_stockin';
```

3. Functional test (requires test data):
- Create an invoice with stock-in transaction
- Attempt to void: `UPDATE invoices SET is_voided = true WHERE id = test_invoice_id;`
- Should fail with: "Cannot void: inventory has been received against this invoice"
  </verify>
  <done>
...
WHERE tgrelid = 'invoices'::regclass
  AND tgname = 'zz_audit_invoice_void_cascade';
```

2. Check function exists with SECURITY DEFINER and search_path:
```sql
SELECT proname, prosecdef, proconfig
FROM pg_proc
WHERE proname = 'audit_invoice_void_cascade';
-- Should show: prosecdef = true, proconfig contains 'search_path=pg_catalog, public'
```

3. Functional test (requires test data with PO, invoice, no stock-in):
- Void an invoice: `UPDATE invoices SET is_voided = true, void_reason = 'Test', voided_by = user_id WHERE id = test_invoice_id;`
- Query audit_logs:
```sql
...
LIMIT 10;
```
- Should see entries for: po_line_items (invoiced_quantity change), purchase_orders (status_change)

4. Verify Balance in Hand unchanged (by design):
```sql
-- Before void: record qmhq.balance_in_hand
-- After void: balance_in_hand should be the same (PO commitment unchanged)
SELECT id, balance_in_hand FROM qmhq WHERE id = affected_qmhq_id;
```
  </verify>
  <done>
...
1. Setup test data:
   ...
   - Create an invoice against the PO (status should change to partially_invoiced)
   - Do NOT create stock-in transactions
...
```sql
-- Delete stock-in for test (or use invoice without stock-in)
...
-- Check audit trail
SELECT entity_type, action, changes_summary
FROM audit_logs
WHERE changes_summary LIKE '%void%'
...
4. Verify VOID requirements:
   ...
   - VOID-02: Balance in Hand unchanged (by design - PO commitment unchanged when invoice voided)
...
- [ ] Balance in Hand correctly remains unchanged after invoice void (verified by design)
</success_criteria>
