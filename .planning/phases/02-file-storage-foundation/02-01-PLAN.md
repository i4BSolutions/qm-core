...
<objective>
Create the database infrastructure for file attachments including metadata table, storage bucket, RLS policies, and cascade/cleanup mechanisms.

Purpose: Establishes the foundational schema that all file operations will rely on, with security policies mirroring entity access.
Output: Three SQL migrations that when applied create a fully RLS-protected file storage system.
</objective>
...
3. **Cleanup helper functions (for Edge Function or scheduled job):**
   ```sql
...
Test functions: SELECT * FROM get_expired_file_paths() runs without error.
  ...
  <done>
Cascade soft-delete triggers active on QMRL and QMHQ. Cleanup functions ready for scheduled purge job.
  </done>
</task>
...
- Cleanup functions ready for scheduled purge
</success_criteria>
