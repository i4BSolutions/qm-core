...
must_haves:
  truths:
    ...
    - "Panel starts visible/expanded on each step (resets per step)"
  artifacts:
...
2. Add panel state (per user decision: reset per step, starts visible on desktop):
```typescript
// Panel starts visible on desktop (>= 768px), closed on mobile
// No sessionStorage persistence - panel resets per step per user decision
...
1. Add imports
2. Add panel state (no sessionStorage - resets per step per user decision)
...
**Note:** No sessionStorage for panel state - user decided panel should reset (start expanded) on each step.
  </action>
  <verify>
- `npm run type-check` passes
- `npm run lint` passes
...
5. **Step 2 verification:**
   ...
   - Panel should start visible/expanded on step 2 (resets per step)
   - Even if closed on step 1, panel should be open again on step 2
...
7. Panel resets to visible/expanded on each step (per user decision)
</verification>
