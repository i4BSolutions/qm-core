...
must_haves:
  truths:
    ...
    - "Cross-cutting tables (file_attachments, comments) enforce parent entity permissions — a user blocked on qmrl cannot see QMRL attachments or comments"
    - "handle_new_user() trigger no longer references the users.role column"
  artifacts:
...
**File attachments (inherits parent entity permission per CONTEXT.md):**

`file_attachments` has `entity_type` (text: 'qmrl' or 'qmhq') and `entity_id` columns. Per the locked decision "cross-cutting tables inherit parent entity's permission," the RLS must check has_permission on the parent resource derived from entity_type.

First, create a helper function to map entity_type text to permission_resource enum:
```sql
...
- SELECT: `USING (deleted_at IS NULL AND has_permission(attachment_entity_resource(entity_type), 'view'))` — can only view attachments for entities user has at least view permission on
- INSERT: `WITH CHECK (has_permission(attachment_entity_resource(entity_type), 'edit'))` — can only attach to entities user has edit permission on
- UPDATE: `USING ((has_permission('admin', 'edit') OR uploaded_by = auth.uid()) AND has_permission(attachment_entity_resource(entity_type), 'view'))` — admin or original uploader, but still needs parent view
- DELETE: `USING (has_permission('admin', 'edit'))` — hard delete admin only

**Comments (inherits parent entity permission per CONTEXT.md):**

`comments` has `entity_type` (text: 'qmrl', 'qmhq', 'po', 'invoice') and `entity_id` columns. Same pattern — check parent entity permission.

First, create a helper function to map comment entity_type text to permission_resource enum:
```sql
...
Then create policies:
- SELECT: `USING (deleted_at IS NULL AND has_permission(comment_entity_resource(entity_type), 'view'))` — can only see comments on entities user has view permission on
- INSERT: `WITH CHECK (has_permission(comment_entity_resource(entity_type), 'view'))` — can comment on entities user can at least view (not requiring edit, since commenting is a lightweight action)
- UPDATE: `USING (author_id = auth.uid() AND NOT public.comment_has_replies(id) AND has_permission(comment_entity_resource(entity_type), 'view'))` — own comments without replies, still needs parent view
...
1. **Update `handle_new_user()` trigger function** — Remove the `role` column from the INSERT statement BEFORE dropping the column. The function currently inserts `(id, email, full_name, role)` with `'qmrl'` as the default role. Replace with an INSERT that only sets `(id, email, full_name)`:
```sql
...
2. Drop `get_user_role()` function
3. Drop `users.role` column
4. Drop `user_role` enum type

Order matters: update handle_new_user() first (removes role reference), then drop the function, column, and enum.

```sql
-- handle_new_user() updated above (step 1)
...
- File_attachments and comments policies check parent entity permission via helper functions (attachment_entity_resource, comment_entity_resource)
- Updates helper functions (can_view_sor_request, delete_file_attachment) to use permission checks instead of role checks
- Updates handle_new_user() to remove the role column from the INSERT before dropping the column
...
- Contains NO verification DO block (verification is done through testing per CONTEXT.md decision)
  </done>
</task>
...
3. All CREATE POLICY statements use `has_permission()` (except user_permissions which uses direct subquery, and audit_logs which use `true`)
...
- Cross-cutting tables (file_attachments, comments) enforce parent entity permissions via entity_type mapping
- The legacy role column, enum, and function are dropped
- handle_new_user() trigger updated to not reference the dropped role column
