...
must_haves:
  truths:
    ...
    - "roleNavigation maps exactly 3 roles to their allowed route arrays"
...
<task type="auto">
  ...
  **Step 2: Update lib/hooks/use-permissions.ts**
  ...
  - `qmhq`: admin=CRUD, qmrl=R, qmhq=CRUD
  ...
  - `qmrl`: /dashboard, /qmrl, /qmhq, /item (RBAC-07: no PO/Invoice/Inventory)
...
<task type="auto">
  ...
  1. Dashboard, QMRL, QMHQ, Items — keep no `roles` (visible to all, same as current)

  2. Purchase Orders: change `roles: ["admin", "quartermaster", "finance", "proposal"]` to `roles: ["admin", "qmhq"]`

  3. Invoices: change `roles: ["admin", "quartermaster", "finance", "inventory", "proposal"]` to `roles: ["admin", "qmhq"]`

  4. Inventory: change `roles: ["admin", "quartermaster", "inventory"]` to `roles: ["admin"]` (admin-only for stock management; QMHQ accesses stock-out-requests via direct URL, not inventory nav)

  5. Warehouses: change `roles: ["admin", "quartermaster", "inventory", "proposal"]` to `roles: ["admin", "qmhq"]`

  6. Admin section: keep `roles: ["admin"]` (unchanged)
  ...
  Note: The QMHQ nav item currently has no `roles` array (visible to all). Per RBAC-07, QMRL users cannot access QMHQ pages. However, RBAC-09 says QMHQ users can view QMRLs (read-only). The research notes that QMHQ should remain visible to all since the sidebar is defensive only — server-side page guards (Plan 02) will enforce actual access. Leave QMHQ without roles restriction for now; Plan 02 will add server guards.

  Actually, re-evaluating: RBAC-07 says QMRL cannot access QMHQ pages. The sidebar should hide QMHQ for QMRL users. Update: Add `roles: ["admin", "qmhq"]` to the QMHQ nav item. But RBAC-09 says QMHQ can view QMRLs — QMRL is already visible to all, so no change needed there.

  Wait: The current setup shows QMHQ to all. If we restrict it, QMRL users won't see the QMHQ nav link. But the success criteria #1 says "cannot access QMHQ... pages." The navigation hiding is the UX layer for this. So add `roles: ["admin", "qmhq"]` to QMHQ nav item.
  ...
  <done>
  Sidebar navigation items use only "admin" and "qmhq" role values. QMRL users see: Dashboard, QMRL, Items. QMHQ users see: Dashboard, QMRL, QMHQ, Purchase Orders, Invoices, Warehouses, Items. Admin sees all including Admin section.
  </done>
</task>
