...
must_haves:
  truths:
    ...
    - "If permission save fails, the user creation is rolled back and an error is shown"
  artifacts:
...
**Set All handling:**
- PermissionMatrix already has an `onSetAll` prop (designed in Plan 01). Pass an `onSetAll` callback from the dialog.
- In the callback, show `window.confirm("Set all 16 resources to [Level]? This will overwrite all current selections.")`
- On confirm, loop through all 16 resources and set them to the requested level in the local permissions state.
...
3. Upsert the permissions with the admin-specified levels:
   ...
   ```
4. If permission upsert fails, treat it as a hard error to maintain atomicity (PERM-03 requires permissions at creation time):
   - Delete the invited user via `supabaseAdmin.auth.admin.deleteUser(inviteData.user.id)` to roll back
   - Return 500 with `{ error: 'Failed to set permissions. User creation rolled back. Please try again.' }`
   - Log the original permission error for debugging
...
Run `npm run type-check` — no TypeScript errors. Run `npm run build` — no build errors. Confirm the route accepts permissions array and upserts to user_permissions table.
  ...
  <done>
The invite-user API accepts a permissions array with 16 entries, validates completeness, upserts permission rows for the new user after invitation, and rolls back user creation if permission save fails. Backward compatible if permissions are omitted.
  </done>
</task>
