...
<objective>
Build the permission matrix component and wire it into the existing user management page as a Permissions dialog, enabling admins to view and edit per-user permissions for all 16 resources.
...
**Note:** CONTEXT.md specifies "Permission matrix lives as a tab on the existing user detail page." No `/admin/users/[id]` detail page exists in the codebase — the user management UI is a single list page with dialog-based editing. The Dialog approach is the equivalent implementation: a dedicated modal accessible from the user list dropdown, functioning as the "tab" for per-user permission management.
</objective>
...
interface PermissionMatrixProps {
  ...
  onSetAll?: (level: PermissionLevel) => void;  // parent handles confirmation + bulk update
...
- If `onSetAll` prop is provided, clicking any Set All button calls `onSetAll(level)` — the parent is responsible for showing a confirmation dialog and then bulk-updating permissions
- If `onSetAll` is NOT provided, fall back to calling `onChange` for every resource (16 calls)
