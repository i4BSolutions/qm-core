...
- Wrap the filter dropdowns (Category and Assigned selects) in `<div className="hidden md:flex items-center gap-4">...</div>`
  ...
  <div className="flex md:hidden">
    <Popover>
      ...
      <PopoverContent className="w-64 space-y-3" align="start">
        {/* Category select and Assigned select rendered here for mobile */}
      </PopoverContent>
    </Popover>
  </div>
  ```
...
Search (left, fixed width via FilterBar.Search) | Status filter (optional, keep Category) | Assigned Person dropdown | Category dropdown | ... | Card/list toggle (right, ml-auto)

Actually per current QMRL, there is no status filter dropdown - it uses Kanban columns for status grouping. Keep that. The toolbar is:
Search | Category | Assigned Person | [toggle]
