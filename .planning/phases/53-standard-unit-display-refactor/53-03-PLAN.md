...
must_haves:
  truths:
    ...
    - "Warehouse inventory rows show per-item standard unit name inline (e.g. '120 pcs', '50 kg')"
  ...
  artifacts:
    - path: "app/(dashboard)/warehouse/[id]/page.tsx"
      provides: "Warehouse detail with per-item unit names and no aggregate standard stock KPI"
...
<objective>
Refactor inventory, warehouse, QMHQ, and stock-out display components to use per-item standard unit names. Remove all aggregate standard qty totals. Add conversion rate live preview to stock-in and stock-out request forms. Update stock-out PDF to use per-item unit names.
...
5. Remove the `standard_stock` field from `WarehouseInventoryItem`. Remove the `standard_stock` aggregation logic from the transaction processing loop (lines that do `inv.standard_stock += ...` and `inv.standard_stock -= ...`).
...
8. Remove the "Total Standard Stock" display in the KPI section (the `{unitName && ...}` block under Total Units that shows aggregated standard stock).
9. In the inventory table, the current_stock cell currently shows standard_stock below it using `{unitName && ...}`. Replace this: instead of showing aggregate `standard_stock`, this cell should show per-item unit info. Since individual inventory rows aggregate multiple transactions (potentially with different conversion rates), and the decision says "standard qty is strictly per-line-item/per-row display only" -- for warehouse inventory rows, we should NOT show standard stock aggregation. The per-row standard qty concept doesn't apply to warehouse aggregated stock levels. Remove the standard_stock display from inventory rows. Keep only the `current_stock` display and the unit from `item_unit` or `standard_unit_name`.
...
5. This means each item in the progress display shows its OWN unit name.
  ...
  <verify>
    ...
    - `grep -c "standard_stock" app/(dashboard)/warehouse/[id]/page.tsx` returns 0
    - `grep "standard_unit" app/(dashboard)/warehouse/[id]/page.tsx` shows join query
  </verify>
  <done>Warehouse detail has no aggregate standard stock KPI, per-transaction standard qty shows per-item unit name. Inventory dashboard shows per-item unit names. QMHQ detail uses per-item unit names throughout. No code references useStandardUnitName.</done>
</task>
...
- No aggregate standard qty totals exist anywhere
...
- No aggregate/summed standard quantities exist in any view
